/* Changed by ANDREI SHUMAK | 281023 */
...
FORWARD _PROTOTYPE( PROCES_GROUP do_getgroup, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETGR:  	r = do_getgr(&m);	break;
	    case SYS_SETGR:  	r = do_setgr(&m);	break;
	    default:		r = E_BAD_FCN;
	}
 ...
 *				find_proces				    *
 *==========================================================================*/
PRIVATE void find_proces(rp, prid)
register struct proc *rp;
int prid;
{
	...
	for(I = NR_TASKS; I < NR_TASKS + NR_PROCS; I++)
		if(proc[I].p_pid == prid && proc[I].p_priority != PPRI_NONE)
		{
			rp = proc[I];
			return;		
		}
 ...
 *				do_getgr				    *
 *==========================================================================*/
PRIVATE PROCES_GROUP do_getgr(m_ptr)
message *m_ptr;
{
	register struct proc *rp;
	if((rp = find_proces(m_ptr->m1_i1)) != NIL_PROC)
		return rp->gr;
	return EINVAL;
}
/*==========================================================================*
 *				do_setgr				    *
 *==========================================================================*/
PRIVATE int do_setgr(m_ptr)
message * m_ptr;
{
	register struct proc *rp;
	GROUP temp = m_ptr->m2_l2;
	if((rp = find_user_proc_by_pid(m_ptr->m1_i1)) == NIL_PROC)
		return EINVAL;
	rp->gr = temp;	
	return 1;
}
