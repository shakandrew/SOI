/* Changed by ANDREI SHUMAK | 281023 */s
 ...
 */
	...
	for(a = 0; a < 3; a++)
	{
		xp = rdy_head[USER_Q]->p_nextready;
		if(temp == GR_A)	temp = GR_B;
		if(temp == GR_B)	temp = GR_C;
		if(temp == GR_C)	temp = GR_A;
  	...
  	if(amount == 1)
	{
		helper = rdy_head[USER_Q];
		while (helper->p_nextready != xp)	helper = helper->p_nextready;

		if(helper == rdy_head[USER_Q])
		{
			...
			if(xp == rdy_tail[USER_Q]) 
			{
				helper->p_nextready = NIL_PROC;
				rdy_tail[USER_Q] = helper;
				xp->p_nextready = rdy_head[USER_Q];
				rdy_head[USER_Q] = xp;
			}
			else 
			{
				...
				helper->p_nextready = xp->p_nextready;
				xp->p_nextready = rdy_head[USER_Q];
				rdy_head[USER_Q] = xp;
		}
 	}
