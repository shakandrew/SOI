...
{
	register struct hole *tmp;
	phys_clicks buffer[NR_HOLES*2+1];
	unsigned int nbytes, i;
	nbytes = mm_in.m1_i1;
	for(i = 0, tmp = hole_head; i < (nbytes/sizeof(unsigned int))-2 && tmp != NIL_HOLE &&
		tmp->h_base < swap_base; tmp = tmp->h_next)
		{
			buffer[i++] = tmp->h_len;
			buffer[i++] = tmp->h_base;
		}
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *max, *max_prev;
  ...
  else
  {
	...
	if(max->h_len >= clicks) {
		...
		/* Delete the hole if used up completely. */
		if (max->h_len == 0) del_slot(max_prev, max);
